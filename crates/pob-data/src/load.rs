...
use crate::{Gem, ItemBase, LoadError, PassiveTree, Skill, SkillSet};
...
pub fn load_skill_file(json: &str) -> Result<SkillSet, LoadError> {
    // Each per-category JSON is `{ skill_id: Skill }`.
    let raw: IndexMap<String, Skill> = serde_json::from_str(json)?;
    Ok(raw)
}
